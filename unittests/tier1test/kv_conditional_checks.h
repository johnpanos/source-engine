...
inline bool TestSymbol( const char *pSymbol, void *pContext )
{
	...
	static const char *const s_TrueSymbols[] = {
	    "WIN32", "LINUX", "POSIX", "ENGLISH", "WIN32WIDE", "WIN32HIDEF" };
...
inline void CheckAll( EvaluatorFn evaluate, Tally &tally )
{
	static const Case s_Cases[] = {
	    ...
	    { "[($WIN32 && $WIN32HIDEF) || ($GAMECONSOLE && ($GAMECONSOLEWIDE && $GAMECONSOLEHIDEF))]",
	        true, true },
	    { "[($WIN32 && !$WIN32HIDEF) || ($GAMECONSOLE && !($GAMECONSOLEWIDE && "
	      "$GAMECONSOLEHIDEF))]",
	        true, false },
	...
	for ( const Case &test : s_Cases )
	{
		...
		const std::string what =
		    std::string( test.expression ) + ( test.valid ? " is valid" : " is rejected" );
		tally.Check( bValid == test.valid, what.c_str() );
		const std::string value =
		    std::string( test.expression ) + ( test.value ? " is true" : " is false" );
	...
	bool bResult = true;
	tally.Check( !evaluate( longSymbol.c_str(), TestSymbol, nullptr, bResult ) && !bResult,
	    "oversized symbol is rejected" );

	// No expression at all.
	bResult = true;
	tally.Check( !evaluate( nullptr, TestSymbol, nullptr, bResult ) && !bResult,
	    "null expression is rejected" );
	...
	tally.Check( evaluate( "[!$WIN32]", nullptr, nullptr, bResult ) && bResult,
	    "missing resolver reads symbols as false" );
}
