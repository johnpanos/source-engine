...
public:
	...
	bool Connect( CreateInterfaceFn ) override { Record( "C" ); return fail != 1; }
	void Disconnect() override { Record( "D" ); }
	InitReturnVal_t Init() override { Record( "I" ); return fail == 3 ? INIT_FAILED : INIT_OK; }
	...
	VideoSystemFeature_t GetSupportedFeatures() override { return VideoSystemFeature::PLAY_VIDEO_FILE_IN_MATERIAL; }
	const char *GetVideoSystemName() override { return name; }
	bool InitializeVideoSystem( IVideoCommonServices * ) override { Record( "V" ); return fail != 2; }
	bool ShutdownVideoSystem() override { Record( "X" ); return true; }
	VideoResult_t VideoSoundDeviceCMD( VideoSoundDeviceOperation_t, void *, void * ) override { return VideoResult::SUCCESS; }
	int GetSupportedFileExtensionCount() override { return 1; }
	const char *GetSupportedFileExtension( int ) override { return ".test"; }
	VideoSystemFeature_t GetSupportedFileExtensionFeatures( int ) override { return VideoSystemFeature::PLAY_VIDEO_FILE_IN_MATERIAL; }
	VideoResult_t PlayVideoFileFullScreen( const char *, void *, int, int, int, int, bool, float, VideoPlaybackFlags_t ) override { return VideoResult::SUCCESS; }
	IVideoMaterial *CreateVideoMaterial( const char *, const char *, VideoPlaybackFlags_t ) override { return nullptr; }
	...
	VideoResult_t CheckCodecAvailability( VideoEncodeCodec_t ) override { return VideoResult::SUCCESS; }
...
IVideoSubSystem *CreateA() { return &a; }
IVideoSubSystem *CreateB() { return &b; }
IVideoSubSystem *CreateMissing() { return nullptr; }
#if defined( VIDEO_SERVICE_INTEGRATION )
...
{
	if ( std::strcmp( name, "VFileSystem017" ) == 0 ||
	     std::strcmp( name, "VMaterialSystem080" ) == 0 )
...
bool TestLinkedServices( VideoProviderDescriptor *descriptors )
{
	...
	passed &= Check( built && built->count >= 0, "actual service exposes its build-selected catalog" );
	VideoProviderCatalog catalog = { descriptors, 2 };
	for ( int repeat = 0; repeat < 2; ++repeat )
	{
		calls.clear();
		IVideoServices *video = VideoServices_CreateWithProviders( &catalog );
		passed &= Check( video && video == VideoServices_Create(), "legacy entry point shares configured service" );
		if ( !video )
			return false;
		passed &= Check( video->Connect( Dependencies ) && video->Init() == INIT_OK, "actual service starts linked codecs" );
		...
		passed &= Check( !VideoServices_CreateWithProviders( &catalog ), "actual connected service rejects reconfiguration" );
		video->Shutdown();
		video->Disconnect();
		passed &= Check( video->GetAvailableVideoSystemCount() == 0 &&
		...
		passed &= Check( calls == "aC aV bC bV aI bI bS aS bX bD aX aD ", "actual service balances the provider lifecycle" );
	}
	...
	passed &= Check( !VideoServices_CreateWithProviders( &catalog ), "actual service rejects duplicate selection" );
	descriptors[1] = saved;
...
int main( int argc, char **argv )
{
	...
	VideoProviderDescriptor descriptors[] = {
		{ "bink", VideoSystem::BINK, CreateA, true },
		{ "webm", VideoSystem::WEBM, CreateB, true }
	};
	VideoProviderCatalog catalog = { descriptors, 2 };
	VideoProviderCatalog empty = { nullptr, 0 };
	passed &= Check( !providers.Init(), "Init requires Connect" );
	passed &= Check( providers.Configure( empty ) && providers.Connect( nullptr, nullptr ) && providers.Init(), "explicit optional empty composition starts" );
	passed &= Check( !providers.Connect( nullptr, nullptr ) && !providers.Configure( catalog ), "connected composition cannot be replaced or double connected" );
	...
	passed &= Check( !providers.Configure( malformed ), "oversized count rejected before reading storage" );
	VideoProviderDescriptor saved = descriptors[1];
	descriptors[1].system = descriptors[0].system;
	passed &= Check( !providers.Configure( catalog ), "duplicate system rejected before factory invocation" );
	descriptors[1] = saved;
	...
	{
		calls.clear();
		passed &= Check( providers.Configure( catalog ) && providers.Connect( nullptr, nullptr ), "valid catalog connects" );
		passed &= Check( providers.Count() == 2 && providers.Get( VideoSystem::BINK ) == &a && !providers.Get( -1 ), "only connected providers are visible" );
		passed &= Check( providers.Init() && providers.Init(), "Init is repeat safe" );
		providers.Disconnect();
		passed &= Check( calls == "aC aV bC bV aI bI bS aS bX bD aX aD ", "reverse shutdown and disconnect balance startup" );
		const std::string finished = calls;
		providers.Disconnect();
		providers.Shutdown();
		passed &= Check( calls == finished && providers.Count() == 0, "repeated teardown performs no callbacks" );
	}
	...
	for ( int stage = 1; stage <= 3; ++stage )
	{
		...
		passed &= Check( !initialized && providers.Count() == 0, "required provider failure rolls back entire composition" );
		passed &= Check( std::strcmp( providers.FailedProvider(), "webm" ) == 0, "failure identifies required provider" );
		const char *expected[] = { "", "aC aV bC bD aX aD ", "aC aV bC bV bX bD aX aD ", "aC aV bC bV aI bI bS bX bD aS aX aD " };
		passed &= Check( calls == expected[stage], "failure cleanup includes failed stage and prior providers" );

		calls.clear();
		descriptors[1].required = false;
		passed &= Check( providers.Configure( catalog ) && providers.Connect( nullptr, nullptr ) && providers.Init(), "optional unavailable provider does not fail composition" );
		passed &= Check( providers.Count() == 1 && !providers.Get( VideoSystem::WEBM ), "optional failure is explicitly unavailable" );
		providers.Disconnect();
		descriptors[1].required = true;
	}
	b.fail = 0;
	...
	passed &= Check( providers.Configure( catalog ) && !providers.Connect( nullptr, nullptr ) && providers.Count() == 0, "required null factory result rolls back" );
	passed &= Check( calls == "aC aV aX aD ", "null factory has no invalid callbacks" );
	descriptors[1].create = CreateA;
	calls.clear();
	passed &= Check( providers.Configure( catalog ) && !providers.Connect( nullptr, nullptr ), "factory identity mismatch rejected" );
	passed &= Check( calls == "aC aV aX aD ", "identity mismatch does not touch the wrong provider" );
	descriptors[1] = saved;
