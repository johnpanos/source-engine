...
void CheckPairSelection(
    IRenderBackendProvider &provider, IRenderPresentationBridgeFactory &bridge, Report &report )
{
    ...
    IPresentationHarness &harness, Report &report )
{
	...
	RenderCreateError error;
	IRenderPresentation *presentation =
	    bridge.CreatePresentation( device, *surface, config, &error );
	report.Record( "create.ok", presentation != nullptr && error.status == RenderCreateStatus::kOk,
    ...
    IPresentationHarness &harness, Report &report )
{
	...
	// A skipped frame closes cleanly and the next one presents.
	RenderResourceHandle skipped = kInvalidResource;
	const bool cancelOpened = p->BeginFrame( &skipped ) == RenderPresentStatus::kOk;
	const RenderPresentStatus cancelled = p->CancelFrame();
	report.Record( "frame.cancel",
	    cancelOpened && cancelled == RenderPresentStatus::kOk &&
	        p->CancelFrame() == RenderPresentStatus::kInvalidSequence &&
	        p->Present() == RenderPresentStatus::kInvalidSequence &&
	        RunFrameAllowingRebuild( device, *p, harness, 0 ) == RenderPresentStatus::kOk,
	    "CancelFrame closes a frame without presenting; the next frame presents" );
	...
	for ( const RenderExtent &size : sizes )
	{
		...
		for ( int i = 0; i < 2; ++i )
		{
			const RenderPresentStatus status =
	...
	report.Record( "surface.replaced_recovers",
	    ...
	    "presentation rebuilds against a replacement native surface (status " +
	        std::to_string( static_cast<int>( back ) ) + ", generation " +
	        std::to_string( generation ) + "->" + std::to_string( surface->GetGeneration() ) +
	        ", surface status " + std::to_string( static_cast<int>( surface->GetStatus() ) ) +
	        ")" );

	// Minimized/hidden window: zero drawable extent.
	if ( !harness.CanToggleVisibility() )
	{
		report.Skip( "zero.surface_suspends", "the window system cannot restore a window" );
		report.Skip( "zero.resume_after_surface", "the window system cannot restore a window" );
	}
	else
	{
		...
		report.Record( "zero.surface_suspends",
		    !surface->GetDrawableExtent().IsPresentable() &&
		        ...
		        device.GetState() == RenderDeviceState::kAvailable,
		    "a minimized window reports a zero drawable and suspends presentation (drawable " +
		        ExtentText( surface->GetDrawableExtent() ) + ")" );
		...
		report.Record( "zero.resume_after_surface", shown == RenderPresentStatus::kOk,
		    "presentation resumes when the surface is shown again (status " +
		        std::to_string( static_cast<int>( shown ) ) + ", drawable " +
		        ExtentText( surface->GetDrawableExtent() ) + ")" );
	}
    ...
    IPresentationHarness &harness, Report &report )
{
	...
	// back buffer. The frame is cancelled, not presented: a native present may
	// wait on the CPU for held work (Mesa WSI does), which a test must not do.
	RenderResourceHandle oldBackBuffer = kInvalidResource;
	RenderPresentStatus heldFrame = p->BeginFrame( &oldBackBuffer );
	if ( heldFrame == RenderPresentStatus::kOk )
	{
		SubmitFrameWork( device, harness, oldBackBuffer, 1 );
		heldFrame = p->CancelFrame();
	}
    ...
    IPresentationHarness &harness, Report &report )
{
	...
	for ( uint32_t frame = 0; frame < 3; ++frame )
		for ( IRenderPresentation *p : live )
			interleaved = interleaved && RunFrameAllowingRebuild( device, *p, harness, frame ) ==
	...
	for ( IRenderPresentation *p : live )
		if ( p )
			survivorsPresent = survivorsPresent && RunFrameAllowingRebuild( device, *p, harness,
			                                           9 ) == RenderPresentStatus::kOk;
	...
	if ( last != middle )
	{
		...
		RenderCreateError error;
		IRenderPresentation *onDestroyed = bridge.CreatePresentation(
		    device, *surfaces[last], RenderPresentationConfig(), &error );
	...
	for ( size_t i = 0; i < live.size(); ++i )
		if ( live[i] && i != last )
			othersAfterLoss = othersAfterLoss && RunFrameAllowingRebuild( device, *live[i], harness,
			                                         10 ) == RenderPresentStatus::kOk;
    ...
    IPresentationHarness &harness, Report &report )
{
	...
	report.Record( "loss.device_lost", presented && simulated && lost == RenderPresentStatus::kLost,
	...
	IRenderPresentation *after = recovered ? bridge.CreatePresentation( *device, *surface,
	                                             RenderPresentationConfig(), nullptr )
	                                       : nullptr;
