...
SHADER_PARAM(
    DETAIL1SCALE, SHADER_PARAM_TYPE_FLOAT, "1.0", "scale detail1 as multiplier of base UVs" )
SHADER_PARAM( DETAIL1FRAME, SHADER_PARAM_TYPE_INTEGER, "0", "frame number for detail1" )
SHADER_PARAM( DETAIL1BLENDMODE, SHADER_PARAM_TYPE_INTEGER, "0",
...
SHADER_PARAM( DETAIL1TEXTURETRANSFORM, SHADER_PARAM_TYPE_MATRIX,
    "center .5 .5 scale 1 1 rotate 0 translate 0 0", "detail1 texcoord transform" )

SHADER_PARAM( DETAIL2, SHADER_PARAM_TYPE_TEXTURE, "shader/BaseTexture", "detail map 2" )
SHADER_PARAM(
    DETAIL2SCALE, SHADER_PARAM_TYPE_FLOAT, "1.0", "scale detail1 as multiplier of base UVs" )
SHADER_PARAM( DETAIL2FRAME, SHADER_PARAM_TYPE_INTEGER, "0", "frame number for detail1" )
SHADER_PARAM( DETAIL2BLENDMODE, SHADER_PARAM_TYPE_INTEGER, "0",
...
SHADER_PARAM( DETAIL2TEXTURETRANSFORM, SHADER_PARAM_TYPE_MATRIX,
    "center .5 .5 scale 1 1 rotate 0 translate 0 0", "detail1 texcoord transform" )

SHADER_PARAM( TANGENTTOPACITYRANGES, SHADER_PARAM_TYPE_VEC4, "[1 0.9 0 0.6]",
    "enables view-based opacity falloff based on tangent t direction, great for cylinders, "
    "includes last term for scaling backface opacity" )
SHADER_PARAM( TANGENTSOPACITYRANGES, SHADER_PARAM_TYPE_VEC4, "[1 0.9 0 0.6]",
    "enables view-based opacity falloff based on tangent s direction, great for cylinders, "
    "includes last term for scaling backface opacity" )
SHADER_PARAM( FRESNELOPACITYRANGES, SHADER_PARAM_TYPE_VEC4, "[1 0.9 0 0.6]",
    "enables fresnel-based opacity falloff, includes last term for scaling backface opacity" )
SHADER_PARAM( NEEDSTANGENTT, SHADER_PARAM_TYPE_BOOL, "0",
    "don't need to set this explicitly, it gets set when tangenttopacityranges is defined" )
SHADER_PARAM( NEEDSTANGENTS, SHADER_PARAM_TYPE_BOOL, "0",
    "don't need to set this explicitly, it gets set when tangentSopacityranges is defined" )
SHADER_PARAM( NEEDSNORMALS, SHADER_PARAM_TYPE_BOOL, "0",
...
SHADER_DRAW
{
	...
	SetupVarsSolidEnergy( info );
	DrawSolidEnergy(
	    this, params, pShaderAPI, pShaderShadow, info, vertexCompression, pContextDataPtr );
}
END_SHADER
