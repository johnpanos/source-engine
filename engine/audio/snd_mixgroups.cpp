...
//
// Purpose: Sound mixers: mix group rules, sound mixers, mix layers and layer
//          triggers from scripts/soundmixers.txt.
//
// This is the CS:GO-era mixer that Portal 2 ships with (its soundmixers.txt
// has MixGroups, SoundMixers with vol/level/dsp/solo/mute per group,
// MixLayers and LayerTriggers). It replaces the Half-Life 2 mixer that lived
// in snd_dma.cpp, which misread Portal 2's file. It also reads the
// Half-Life 2 layout (GROUPRULES and top-level mixers with one volume per
// group), so Portal and Half-Life 2 keep their mix.
//
//===========================================================================//
...
void MXR_SetSoloActive(void)
{
	...
	if ( !snd_disable_mixer_solo.GetBool() && g_isoundmixer >= 0 )
	{
...
void MXR_GetMixGroupFromSoundsource( channel_t *pchan, SoundSource soundsource, soundlevel_t soundlevel )
{
	...
	V_strncpy( sndname, pchan->sfx->getname(), sizeof( sndname ) );
	...
	// check for player
	bool bIsPlayer = g_pSoundServices->IsPlayer( soundsource );
...
enum
{
	...
	MXRPARSE_LEGACYMIXERGROUPS	// Half-Life 2 era file: a sound mixer at the top level

};
#define MIXGROUPS_STRING "MixGroups"
#define LEGACY_MIXGROUPS_STRING "GROUPRULES"	// Half-Life 2 era name of the MixGroups block
...
bool MXR_LoadAllSoundMixers( void )
{
	...
	while (1)
	{
		...
		else if ( com_token[0] == CHAR_RIGHT_PAREN )
		{
			...
			else if(currentMxrParse == MXRPARSE_LEGACYMIXERGROUPS)
			{
				currentMxrParse = MXRPARSE_NONE;
				g_csoundmixers++;
			}
		...
		{
			if (!Q_strcmp( com_token, MIXGROUPS_STRING ) || !Q_strcmp( com_token, LEGACY_MIXGROUPS_STRING ) )
			{
			...
			{
				// Half-Life 2 era soundmixers.txt (Portal, HL2): each sound
				// mixer is a top level block of "group" "volume" lines.
				...
				currentMxrParse = MXRPARSE_LEGACYMIXERGROUPS;
				continue;
			}
		}
		else if ( currentMxrParse == MXRPARSE_LEGACYMIXERGROUPS && currentParseLevel == 1 )
		{
			pstart = MXR_ParseSoundMixer( pstart, &g_soundmixers[g_csoundmixers] );
		}
