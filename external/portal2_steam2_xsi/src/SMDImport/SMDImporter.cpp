...
bool	SMDImporter::Parse ( FILE *f )
{
	...
	try 
	{
		...
		while (1)
		{
			char *currenttok = strtok (NULL, seps);
...
bool SMDImporter::ImportTriangles ()
{
	...
	while (1)
	{
		...
		for (int v=0;v<3;v++)
		{
			...
			if ( !jumpNext)
				rigid = atoi(strtok (NULL, seps));

			l_pTriangle->m_pVertex.Extend(1);
			SMDVertex* l_pVert = &l_pTriangle->m_pVertex[l_pTriangle->m_pVertex.GetUsed()-1];
			l_pVert->m_vPosition.PutX ( atof(strtok (NULL, seps)) );
			l_pVert->m_vPosition.PutY ( atof(strtok (NULL, seps)) );
			l_pVert->m_vPosition.PutZ ( atof(strtok (NULL, seps)) );
			l_pVert->m_vNormal.PutX ( atof(strtok (NULL, seps)) );
			l_pVert->m_vNormal.PutY ( atof(strtok (NULL, seps)) );
			l_pVert->m_vNormal.PutZ ( atof(strtok (NULL, seps)) );
			l_pVert->m_vUV.PutX ( atof(strtok (NULL, seps)) );
			l_pVert->m_vUV.PutY ( atof(strtok (NULL, seps)) );

			token = strtok (NULL, sepswnl);
			if ( isdigit( token[0] ) )
			{
				//
				// import weights
				...
				for (int w=0;w<numWeights;w++)
				{
					...
					l_pVert->m_vWeights[l_pVert->m_vWeights.GetUsed()-1].m_iBoneID = atoi(strtok (NULL, seps));
					l_pVert->m_vWeights[l_pVert->m_vWeights.GetUsed()-1].m_fWeight = (float)atof(strtok (NULL, seps));
		...
		if ( strcmp ( token, "time" ))
		{
			...
			for (int c=0;c<m_pNodes.GetUsed();c++)
			{
				if ( m_pNodes[c]->m_iID == nodeNum )
				{
					...
					l_pKey->m_fTime = (float)currentFrame;
