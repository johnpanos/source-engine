...
{
	vc[0] = va[0] + (float)scale*vb[0];
	vc[1] = va[1] + (float)scale*vb[1];
	vc[2] = va[2] + (float)scale*vb[2];
}
...
// find the major axis
	...
	VectorScale (vup, MAX_TRACE_LENGTH, vup);
	VectorScale (vright, MAX_TRACE_LENGTH, vright);
