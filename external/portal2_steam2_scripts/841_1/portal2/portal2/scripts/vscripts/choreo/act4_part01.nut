...
//Finale
	...
	Dialog[2403] <- { speaker = GLADOS, one = "These are unstable personality cores" nextLine = 2404, nextLineDelay = 4 }
	Dialog[2404] <- { speaker = GLADOS, one = "Very unstable" }

	Dialog[2405] <- { speaker = GLADOS, one = "Look for a maintenance station" two = "So I can reinstall myself into a core" nextLine = 2406, nextLineDelay = 4 }
	Dialog[2406] <- { speaker = GLADOS, one = "Don't put me into one of those spheres though" }

	Dialog[2407] <- { speaker = GLADOS, one = "Here's a maintenance station" nextLine = 2408, nextLineDelay = 4 }
	Dialog[2408] <- { speaker = GLADOS, one = "Place me inside and press the button" }

	Dialog[2409] <- { speaker = GLADOS, one = "We can attach a few" two = "of those bad cores to Wheatley" nextLine = 2410, nextLineDelay = 4 }
	Dialog[2410] <- { speaker = GLADOS, one = "So the replacement procedure can happen" }
	...
	Dialog[2504] <- { speaker = WHEATLEY, one = "Step 2: Destroy the Stalemate Button" }

	Dialog[2505] <- { speaker = WHEATLEY, one = "Step 3: Start the neurotoxins immediately" }

	Dialog[2506] <- { speaker = WHEATLEY, one = "Step 4: Use turrets" }

	Dialog[2507] <- { speaker = WHEATLEY, one = "Step 5: Use bombs" }
	...
	Dialog[2515] <- { speaker = WHEATLEY, one = "Oh no" two = "My turrets!" nextLine = 2516, nextLineDelay = 4 }
	Dialog[2516] <- { speaker = WHEATLEY, one = "Now you die!" }

	Dialog[2517] <- { speaker = GLADOS, one = "The paint pipes are backed up!" }

	Dialog[2520] <- { speaker = GLADOS, one = "Here comes some more blue gel!" }

	Dialog[2521] <- { speaker = GLADOS, one = "Here comes some orange gel!" }
...
function GladosTurretEvent()
{
	SpeakLineVcd ( 2517 )
}

function GladosPaintEvent1()
{
	SpeakLineVcd ( 2520 )
}

function GladosPaintEvent2()
{
	SpeakLineVcd ( 2521 )
}
