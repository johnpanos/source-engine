...
def import_scene(scene):
    ...
    for shape in scene["shapes"]:
        ...
        obj.name = obj.data.name = Path(shape["filename"]).stem
        mesh = obj.data
...
def render_reference(scene, stage, environment, out, samples, scale):
    ...
    pbrt_blender.configure_cycles(samples)
...
def main():
    ...
    receipt = {"status": "pass", "scope": "pbrt-usd-stage",
               ...
               "material_assignments": {Path(shape["filename"]).stem: shape["material"]
                                        for shape in scene["shapes"]},
