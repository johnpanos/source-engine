...
def main():
    ...
    pack_lightmap_uvs(baked, margin=args.margin_texels / args.size)
    extents = {}
    density = {}
    for obj in baked:
        layer = obj.data.uv_layers.get("lightmap_st").data
        uv_area = 0.0
        ...
        world_area = sum(polygon.area for polygon in obj.data.polygons)
        if world_area > 1e-6:
    ...
    pbrt_blender.configure_cycles(args.samples)
    ...
    for material in {slot.material for obj in baked for slot in obj.material_slots}:
