...
CONFIG_KEYS="MAP WIDTH HEIGHT WINDOWED FPS_MAX RENDERER PHYSICS PHYSICS_ARGS JOB_ARGS MAT_ARGS EXTRA_ARGS \
...
PHYSICS_ARGS=-physics_shape_inertia  # RFC 0013 shape inertia; run.conf explains
...
# shellcheck disable=SC2206  # JOB_ARGS and PHYSICS_ARGS are intentionally word-split
job_args=($JOB_ARGS)
...
cmd=(./hl2_launcher -game portal -w "$WIDTH" -h "$HEIGHT"
     -renderer "$RENDERER" -physics "$PHYSICS" "${physics_args[@]}" +fps_max "$FPS_MAX" "${job_args[@]}" "${mat_args[@]}" +map "$MAP")
...
echo "run.sh: booting map=$MAP ${WIDTH}x${HEIGHT} renderer=$RENDERER physics=$PHYSICS $PHYSICS_ARGS"
exec "${cmd[@]}" "${extra[@]}" "$@"
