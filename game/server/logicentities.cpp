...
#ifdef PORTAL2
// Portal 2 port: logic_script from the CS:GO 2017 source leak (logicentities.cpp),
// unchanged apart from this guard and a missing-VM check. The entity script
// support it builds on (CBaseEntity::RunVScripts, m_ScriptScope) exists only
// in the Portal 2 server.
// The retail Portal 2 server also reads the last group from "Group16".
...
public:
	...
	void RunVScripts()
	{
		...
		*/
		...
		// Portal 2 port: without a script VM the base class reports the missing scope.
		if ( iLastMember >= 0 && g_pScriptVM )
		{
...
#ifdef PORTAL2
// Portal 2 port: logic_coop_manager and logic_register_activator, used by the
// retail Portal 2 maps. From the CS:GO source drop (logicentities.cpp); the
