...
// Portal 2 portals each have their own size; Portal 1 portals share one.
#ifdef PORTAL2
#define PORTAL_LOCAL_MINS( pPortal ) ( pPortal )->GetLocalMins()
#define PORTAL_LOCAL_MAXS( pPortal ) ( pPortal )->GetLocalMaxs()
#else
#define PORTAL_LOCAL_MINS( pPortal ) CProp_Portal_Shared::vLocalMins
#define PORTAL_LOCAL_MAXS( pPortal ) CProp_Portal_Shared::vLocalMaxs
#endif
...
bool UTIL_TestForOrientationVolumes( QAngle& vecCurAngles, const Vector& vecCurOrigin, const CProp_Portal* pPortal )
{
	...
	while ( pList )
	{
		...
		if ( IsOBBIntersectingOBB( vecCurOrigin, vecCurAngles, PORTAL_LOCAL_MINS( pPortal ), PORTAL_LOCAL_MAXS( pPortal ), 
...
void CFuncPortalOrientation::OnActivate( void )
{
	...
	if( iPortalCount != 0 )
	{
		...
		for( int i = 0; i != iPortalCount; ++i )
		{
			...
			if( IsOBBIntersectingOBB( pTempPortal->GetAbsOrigin(), pTempPortal->GetAbsAngles(), PORTAL_LOCAL_MINS( pTempPortal ), PORTAL_LOCAL_MAXS( pTempPortal ), 
			GetAbsOrigin(), GetCollideable()->GetCollisionAngles(), GetCollideable()->OBBMins(), GetCollideable()->OBBMaxs() ) )
			{
				QAngle angNewAngles;
				if ( m_bMatchLinkedAngles )
				{
					auto *pLinked = pTempPortal->m_hLinkedPortal.Get();
					if ( !pLinked )
						return;
				...
				#ifdef PORTAL2
				pTempPortal->PlacePortal( pTempPortal->GetAbsOrigin(), angNewAngles, PORTAL_PLACEMENT_SUCCESS );
#else
