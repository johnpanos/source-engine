...
//
// Purpose: env_projectedtexture for Portal 2.
//
// The retail Portal 2 server's CEnvProjectedTexture datadesc (41 entries) is
// the CS:GO base version: brightness scale, a color32 light color with timed
// transitions, light styles and patterns, near/far Z inputs, always-update,
// simple projection and projection size/rotation keys, and the rule that
// turning one projected texture on turns every other one off. This follows the
// local cstrike15_src game/server/env_projectedtexture.cpp, adapted to this
// tree's network helpers. The client half is client/c_env_projectedtexture.cpp
// under PORTAL2. Not original Valve source; the repository's provenance
// warning applies.
//
//=============================================================================//
...
CEnvProjectedTexture::CEnvProjectedTexture( void )
{
	...
	m_iStyle = 0;
	m_iDefaultStyle = 0;
	m_iszPattern = NULL_STRING;
}

// Also used by other entities that parse a "lightcolor"-style key.
...
{
	// Retail (CS:GO base) disables this input.
