...
//
// Purpose: trigger_catapult server entity (keyvalues, inputs, networking,
//          debug overlays and the delayed relaunch of blocked objects). The
//          launch math is shared with the client in
...
// (no original source, pseudocode or debug data exists). Adapted from the TF2
// copy of the Portal 2 catapult (hl2_src game/server/tf/trigger_catapult.cpp);
// the datadesc and send table follow the retail Portal 2 server's
// CTriggerCatapult tables. Not original Valve source; the repository's
...
//-----------------------------------------------------------------------------
// Purpose: Draws the player (green) and physics (yellow) launch arcs
//-----------------------------------------------------------------------------
...
int CTriggerCatapult::DrawDebugTextOverlays( void )
{
	...
	if ( m_debugOverlays & OVERLAY_TEXT_BIT )
	{
		...
		{
			Q_snprintf( tempstr, sizeof(tempstr), "Lower threshold velocity: %.2f", m_flLowerThreshold.Get() );
			EntityText( text_offset, tempstr, 0 );
			text_offset++;

			Q_snprintf( tempstr, sizeof(tempstr), "Upper threshold velocity: %.2f", m_flUpperThreshold.Get() );
			EntityText( text_offset, tempstr, 0 );
			text_offset++;
		}

		Q_snprintf( tempstr, sizeof(tempstr), "Player velocity: %.2f", m_flPlayerVelocity.Get() );
		EntityText( text_offset, tempstr, 0 );
		text_offset++;

		Q_snprintf( tempstr, sizeof(tempstr), "Physics velocity: %.2f", m_flPhysicsVelocity.Get() );
		EntityText( text_offset, tempstr, 0 );
		text_offset++;
...
void CTriggerCatapult::Spawn( void )
{
	...
	m_flLowerThreshold = clamp( m_flLowerThreshold.Get(), 0.0f, 1.0f );
	m_flUpperThreshold = clamp( m_flUpperThreshold.Get(), 0.0f, 1.0f );

	// The client predicts launches, so it needs this trigger while the player can see it
...
//-----------------------------------------------------------------------------
// Purpose: Retries launches that were refused (refire delay, held objects,
//          players trapped under another player)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Thinks once a second, looking for a living player. Once one is
//          found, evaluates whether players pass our trigger filters and
//          networks that down to the client (which cannot run the filters).
//-----------------------------------------------------------------------------
