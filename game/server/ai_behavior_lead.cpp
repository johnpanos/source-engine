...
bool CAI_LeadBehavior::Speak( AIConcept_t speechConcept )
{
	CAI_Expresser *pExpresser = GetOuter()->GetExpresser();
	if ( !pExpresser )
		return false;
	...
	bool bNag = ( FStrEq(speechConcept,TLK_LEAD_COMINGBACK) || FStrEq(speechConcept, TLK_LEAD_CATCHUP) || FStrEq(speechConcept, TLK_LEAD_RETRIEVE) );
	...
	if ( hl2_episodic.GetBool() )
	{
		...
		if ( pAlly )
 			return pAlly->SpeakIfAllowed( speechConcept, GetConceptModifiers( speechConcept ) );
	}
	...
	if ( pExpresser->Speak( speechConcept, GetConceptModifiers( speechConcept ) ) )
	{
