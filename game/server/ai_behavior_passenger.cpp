...
// Purpose: 
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::SpeakIfAllowed( AIConcept_t speechConcept, const char *modifiers /*= NULL*/, bool bRespondingToPlayer /*= false*/, char *pszOutResponseChosen /*= NULL*/, size_t bufsize /*= 0*/ )
{
	// FIXME: Store this cast off?
	CAI_PlayerAlly *pAlly = dynamic_cast<CAI_PlayerAlly *>(GetOuter());
	if ( pAlly != NULL )
		return pAlly->SpeakIfAllowed( speechConcept, modifiers, bRespondingToPlayer, pszOutResponseChosen, bufsize );
