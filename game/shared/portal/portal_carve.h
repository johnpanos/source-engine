...
// Separating the world collision from wall collision by a small amount gets rid
    ...
    int iPlaneCount, float fEpsilon, CPolyhedron **pResults, int iMode, IThreadPool *pPool )
{
	...
	if ( !JobGraphParallelProcess( "PortalCarve::ClipBatch", items.Base(), unsigned( iCount ),
	         &context, &ClipContext_t::Process, static_cast<void ( ClipContext_t::* )()>( NULL ),
    ...
    IThreadPool *pPool )
{
	...
	if ( !ClipBatch(
	         pInputs, iCount, pPlanes, iPlaneCount, fEpsilon, results.Base(), iMode, pPool ) )
		return false;
	for ( int i = 0; i != iCount; ++i )
	{
		if ( results[i] )
    ...
    CUtlVector<CPolyhedron *> *pOut, int *pAppended, int iMode, IThreadPool *pPool )
{
	...
	if ( !ClipBatch(
	         pPieces, iPieceCount, pPlanes, iPlaneCount, fEpsilon, results.Base(), iMode, pPool ) )
		return false;
...
inline void ComputeWallBrushBounds( const Placement_t &placement, Vector *pMins, Vector *pMaxs )
{
	...
	for ( int i = 1; i != 8; ++i )
	{
		...
		if ( ptTest.x < vAABBMins.x )
			vAABBMins.x = ptTest.x;
		if ( ptTest.y < vAABBMins.y )
			vAABBMins.y = ptTest.y;
		if ( ptTest.z < vAABBMins.z )
			vAABBMins.z = ptTest.z;
		if ( ptTest.x > vAABBMaxs.x )
			vAABBMaxs.x = ptTest.x;
		if ( ptTest.y > vAABBMaxs.y )
			vAABBMaxs.y = ptTest.y;
		if ( ptTest.z > vAABBMaxs.z )
			vAABBMaxs.z = ptTest.z;
	}
	*pMins = vAABBMins;
	*pMaxs = vAABBMaxs;
}
    ...
    IThreadPool *pPool )
{
	...
	float fTubeDepthDist = vBackward.Dot(
	    placement.ptCenter + ( vBackward * ( PORTAL_WALL_TUBE_DEPTH + PORTAL_WALL_TUBE_OFFSET ) ) );
	...
	float fFarRightPlaneDistance = placement.vRight.Dot(
	    placement.ptCenter + placement.vRight * ( PORTAL_WALL_FARDIST * 10.0f ) );
	float fFarLeftPlaneDistance =
	    vLeft.Dot( placement.ptCenter + vLeft * ( PORTAL_WALL_FARDIST * 10.0f ) );
	...
	bool bOk = true;
	if (
	    bSimulatingVPhysics ) //if not simulating vphysics, we skip making the entire wall, and just create the minimal tube instead
	{
		if ( iWallBrushCount != 0 )
			bOk = ClipInOrder( pWallBrushes, iWallBrushCount, fPlanes, 1,
			    PORTAL_POLYHEDRON_CUT_EPSILON, &WallBrushPolyhedrons_ClippedToWall, iMode, pPool );
		...
		if ( bOk && WallBrushPolyhedrons_ClippedToWall.Count() != 0 )
		{
			...
			{
				CPolyhedron *pPolyhedron = ClipPolyhedron( WallBrushPolyhedrons_ClippedToWall[i],
				    fSidePlanesOnly, 4, PORTAL_POLYHEDRON_CUT_EPSILON, true );
				if ( pPolyhedron )
				{
			...
			if ( WallBrushPolyhedrons_ClippedToWall.Count() !=
			     0 ) //might have become 0 while removing uncut brushes
			{
