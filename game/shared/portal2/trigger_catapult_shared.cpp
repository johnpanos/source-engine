...
// Adapted from the TF2 copy that identifies its launch logic as Portal 2 code.
// Retail Portal 2's DT_TriggerCatapult names and the server-side reconstruction
// establish the shared fields. This file is not original Portal 2 source.
#include "cbase.h"
...
#include "portal2/portal/c_portal_player.h"
...
Vector CTriggerCatapult::CalculateLaunchVector( CBaseEntity *pVictim, CBaseEntity *pTarget )
{
	...
	float flSpeed =
	    ( pVictim->IsPlayer() ) ? (float)m_flPlayerVelocity : (float)m_flPhysicsVelocity; // u/sec
...
Vector CTriggerCatapult::CalculateLaunchVectorPreserve(
    Vector vecInitialVelocity, CBaseEntity *pVictim, CBaseEntity *pTarget, bool bForcePlayer )
{
#if defined( CLIENT_DLL )
	if ( !GetPredictable() || !pVictim->GetPredictable() )
		return vec3_origin;
	...
	float flVelocity = ( pVictim->IsPlayer() || bForcePlayer ) ? (float)m_flPlayerVelocity
	...
	float flRadical =
	    flVelocity * flVelocity * flVelocity * flVelocity -
	    flGravity * ( flGravity * flDist * flDist - 2.f * flHeight * flVelocity * flVelocity );
...
void CTriggerCatapult::LaunchByTarget( CBaseEntity *pVictim, CBaseEntity *pTarget )
{
	...
	if ( pVictim->IsPlayer() )
	{
		...
		if ( pPlayer )
		{
			...
			pPlayer->SetAirControlSupressionTime( flSupressionTimeInSeconds * 1000.0f );
#if defined( CLIENT_DLL )
			// The client entity API publishes predicted velocity directly.
			pVictim->SetAbsVelocity( vecVelocity );
#else
...
void CTriggerCatapult::OnLaunchedVictim( CBaseEntity *pVictim )
{
	...
	if ( pVictim->IsPlayer() )
	{
		int nRefireIndex = pVictim->entindex();
		m_flRefireDelay.Set( nRefireIndex, gpGlobals->curtime + 0.5f );
	}
	else
	{
		m_flRefireDelay.Set( 0, gpGlobals->curtime + 0.5f );
	}
}
...
void CTriggerCatapult::StartTouch( CBaseEntity *pOther )
{
	...
	{
		Warning( "CTriggerCatapult::StartTouch Trying to store a refire index for an entity( %d ) "
		         "outside the expected range ( < %d ).\n",
	...
	if ( pOther->VPhysicsGetObject() &&
	     ( pOther->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD ) )
	{
	...
	if ( pLaunchTarget )
	{
		...
		if ( m_bUseThresholdCheck )
		{
			...
			if ( flDot >= m_flEntryAngleTolerance )
			{
				...
				if ( ( ( flLaunchSpeed - ( flLaunchSpeed * m_flLowerThreshold ) ) <
				         flVictimSpeed ) &&
				     ( ( flLaunchSpeed + ( flLaunchSpeed * m_flUpperThreshold ) ) >
				         flVictimSpeed ) )
				{
	...
	else
	{
		...
		{
			Warning( "Catapult launch target not networked to client! This will make prediction "
			         "fail! Fix this in the map.\n"
			         "Catapult launch target not networked to client! This will make prediction "
			         "fail! Fix this in the map.\n"
			         "Catapult launch target not networked to client! This will make prediction "
			         "fail! Fix this in the map.\n"
			         "Catapult launch target not networked to client! This will make prediction "
