...
//
//
// Portal 2 port: C_BeamSpotLight (beam_spotlight), from the CS:GO source drop;
// the repository's provenance and distribution warning applies.
// Adapted to the SDK 2013 render color accessors and client entity creation.
//
//===========================================================================//
...
void C_BeamSpotLight::SpotlightCreate(void)
{
...

	{
		...
		// Beam only exists client side. Portal 2 port: register it in the entity
		// list, leaf system and partition (CS:GO only added it to the list).
		if ( !beam || !beam->InitializeAsClientEntityByIndex( beam->GetModelIndex(), RENDER_GROUP_TRANSLUCENT_ENTITY ) )
		{
			if ( beam )
			{
				beam->Release();
