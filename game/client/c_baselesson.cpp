...
//
//
// Imported from the CS:GO 2017 source leak (cstrike15_src; recorded in
// tools/portal2/cs15_imports.txt). The repository's provenance and
// distribution warning applies. "Portal 2 port:" comments mark adaptations.
//============================================================================//
...
#include "vprof.h"
#include "view.h"
#include "vstdlib/IKeyValuesSystem.h" // Portal 2 port: SDK header name is case-sensitive on Linux
#include "isaverestore.h" // Portal 2 port: ISave/IRestore/CDefSaveRestoreOps are not pulled in by cbase.h here
...
//
// Portal 2 port: file-local adapters for CS:GO-era player/weapon/input API
// that this client does not provide.
//
namespace
{
// CS:GO C_BasePlayer::ActivePlayerCombatCharacter() returns the player itself
// unless a mod controls another character; Portal 2 players never do.
inline C_BaseCombatCharacter *InstructorCombatCharacter( C_BasePlayer *pPlayer )
{
	return pPlayer;
}

// CS:GO CInput::ControllerModeActive(); IInput::EnableJoystickMode() is this
// branch's equivalent (console or in_joystick).
inline bool InstructorControllerModeActive()
{
	return input->EnableJoystickMode();
}

// CS:GO C_BaseEntity::GetPlayerName() returns NULL for non-players.
inline const char *InstructorPlayerName( C_BaseEntity *pEntity )
{
	C_BasePlayer *pPlayer = ToBasePlayer( pEntity );
	return pPlayer ? pPlayer->GetPlayerName() : NULL;
}

// CS:GO UTIL_CountNumBitsSet( unsigned int ) from util_shared.cpp.
int UTIL_CountNumBitsSet( unsigned int nVar )
{
	int nNumBits = 0;
	for ( ; nVar; nVar &= nVar - 1 )
		++nNumBits;
	return nNumBits;
}

// CS:GO weapons carry their own reserve ammo; here reserve ammo lives on the
// owning character, keyed by the weapon's primary ammo type.
int InstructorReserveAmmoCount( C_BaseCombatWeapon *pWeapon )
{
	C_BaseCombatCharacter *pOwner = pWeapon->GetOwner();
	int iAmmoType = pWeapon->GetPrimaryAmmoType();
	return ( pOwner && iAmmoType >= 0 ) ? pOwner->GetAmmoCount( iAmmoType ) : 0;
}

int InstructorReserveAmmoMax( C_BaseCombatWeapon *pWeapon )
{
	int iAmmoType = pWeapon->GetPrimaryAmmoType();
	return ( iAmmoType >= 0 ) ? GetAmmoDef()->MaxCarry( iAmmoType ) : 0;
}

// CS:GO C_BasePlayer::Weapon_CanUse() is the shared base default (always true).
inline bool InstructorWeaponCanUse( C_BasePlayer *pPlayer, C_BaseCombatWeapon *pWeapon )
{
	return true;
}
}
...
void CIconLesson::Update( void )
{
	...
	if ( m_fUpdateDistanceTime < gpGlobals->curtime )
	{
		...
		{
			m_fCurrentDistance = InstructorCombatCharacter( pLocalPlayer )->EyePosition().DistTo( pTarget->WorldSpaceCenter() );
		}
...
void CIconLesson::UpdateInactive( void )
{
	if ( m_fUpdateDistanceTime < gpGlobals->curtime )
	{
		...
		{
			m_fCurrentDistance = InstructorCombatCharacter( pLocalPlayer )->EyePosition().DistTo( pIconTarget->WorldSpaceCenter() );
		}
...
{
	if ( IsGameConsole() || InstructorControllerModeActive() )
	{
...
#define DEFINE_GAMEINSTRUCTOR_SYMBOL(name) \
	{ FIELD_CUSTOM, #name, { offsetof(classNameTypedef,name), 0 }, 1, FTYPEDESC_SAVE, NULL, GetGameInstructorSymbolSaveRestoreOps( ), NULL } /* Portal 2 port: two-slot fieldOffset */
...
void CScriptedIconLesson::FireGameEvent( IGameEvent *event )
{
	...
	if( m_bOnlyKeyboard && InstructorControllerModeActive() )
		return;

	if( m_bOnlyGamepad && !InstructorControllerModeActive() )
		return;
...
bool CScriptedIconLesson::ProcessElementAction( int iAction, bool bNot, const char *pchVarName, EHANDLE &hVar, const CGameInstructorSymbol *pchParamName, float fParam, C_BaseEntity *pParam, const char *pchParam )
{
	...
	switch ( iAction )
	{
		...
		case LESSON_ACTION_GET_DISTANCE:
		{
			...
			Vector vVarPos = ( pVarPlayer ? InstructorCombatCharacter( pVarPlayer )->EyePosition() : pVar->WorldSpaceCenter() );
			Vector vParamPos = ( pParamPlayer ? InstructorCombatCharacter( pParamPlayer )->EyePosition() : pParam->WorldSpaceCenter() );
		...
		case LESSON_ACTION_GET_ANGULAR_DISTANCE:
		{
			...
			if ( pVar->IsPlayer() )
			{
				AngleVectors( InstructorCombatCharacter( static_cast< C_BasePlayer* >( pVar ) )->EyeAngles(), &vVarForward, NULL, NULL );
			}
			else
			{
		...
		case LESSON_ACTION_GET_PLAYER_DISPLAY_NAME:
		{
			...
			*pString = InstructorPlayerName( pVar );
		...
		case LESSON_ACTION_WEAPON_AMMO_LOW:
		{
			...
			{
				CBaseCombatWeapon *pBaseCombatWeaponTemp = InstructorCombatCharacter( pBasePlayer )->GetWeapon( iWeapon );
			...
			int iMaxAmmo = InstructorReserveAmmoMax( pBaseCombatWeapon );
			int iPlayerAmmo = InstructorReserveAmmoCount( pBaseCombatWeapon );
		...
		case LESSON_ACTION_WEAPON_CAN_USE:
		{
			...
			bool bCanEquip = InstructorWeaponCanUse( pBasePlayer, pBaseCombatWeapon );
		...
		case LESSON_ACTION_GET_POTENTIAL_USE_TARGET:
		{
			...
			pHandle->Set( C_BaseEntity::Instance( pBasePlayer->GetUseEntity() ) /* Portal 2 port: CS:GO GetPotentialUseEntity() returns GetUseEntity() */ );
