...
// Portal 2 port: Portal 2's matchmaking extension keeps the packed lesson
// counts in TitleData1 (CS:GO moved them to TitleData3).
#if defined( PORTAL2 )
typedef TitleData1::GameInstructorData_t::LessonInfo_t GameInstructorLessonInfo_t;
#else
typedef TitleData3::GameInstructorData_t::LessonInfo_t GameInstructorLessonInfo_t;
#endif
...
bool C_GameInstructor::ReadSaveData( void )
{
	...
	// Portal 2 port: this build has no matchmaking framework (g_pMatchFramework
	// is NULL), so profile TitleData is unavailable. Read the counts from the
	// KeyValueSaver file that WriteSaveData already maintains instead.
	if ( !g_pMatchFramework )
		return ReadSaveDataFromFile();
...
void C_GameInstructor::KeyValueBuilder( KeyValues *pKeyValues )
{
	...
	// Portal 2 port: without a matchmaking framework, save the counts into the
	// KeyValueSaver file (see ReadSaveDataFromFile).
	if ( !g_pMatchFramework )
	{
		KeyValueBuilderToFile( pKeyValues );
		return;
	}
...
// Portal 2 port: file-backed lesson counts used when g_pMatchFramework is NULL.
// Layout: one subkey per lesson name holding "display" and "success" counts.
bool C_GameInstructor::ReadSaveDataFromFile( void )
{
	m_bHasLoadedSaveData = true;

	KeyValues *pData = KeyValueSaver().GetKeyValues( GAMEINSTRUCTOR_SAVE_FILE );
	if ( pData )
	{
		for ( KeyValues *pLessonKey = pData->GetFirstTrueSubKey(); pLessonKey; pLessonKey = pLessonKey->GetNextTrueSubKey() )
		{
			CBaseLesson *pLesson = GetLesson_Internal( pLessonKey->GetName() );
			if ( !pLesson )
				continue;

			pLesson->SetDisplayCount( pLessonKey->GetInt( "display" ) );
			pLesson->SetSuccessCount( pLessonKey->GetInt( "success" ) );
		}
	}
...
void C_GameInstructor::KeyValueBuilderToFile( KeyValues *pKeyValues )
{
	for ( int i = 0; i < m_Lessons.Count(); ++i )
	{
		CBaseLesson *pLesson = m_Lessons[ i ];
		...
		KeyValues *pLessonKey = pKeyValues->FindKey( pLesson->GetName(), true );
		pLessonKey->SetInt( "display", pLesson->GetDisplayCount() );
		pLessonKey->SetInt( "success", pLesson->GetSuccessCount() );
	}
}
