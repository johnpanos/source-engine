...
//
//
// Portal 2 port: the movie_display_screen VGUI screen for vgui_movie_display, from
// the CS:GO source drop; the repository's provenance and distribution warning applies.
// Playback goes through this engine's IVideoServices (g_pVideo) instead of the
// CS:GO Bink interface; a build without a video provider reports that once.
//
//=====================================================================================//
...
private:
	...
	// Movie playback info. Portal 2 port: an IVideoServices material instead of
	// a CS:GO Bink handle; NULL means no movie.
...
void CMovieDisplayScreen::UpdateMovie( void )
{
	...
	if ( bScreenActive != m_bLastActiveState )
	{
		if ( bScreenActive )
		{
			...
			{
				// Bink's SetFrame( 1 ) restarts at the first frame.
				m_pVideoMaterial->SetTime( 0.0f );
			}
...
bool CMovieDisplayScreen::BeginPlayback( const char *pFilename )
{
	...
	if ( m_pVideoMaterial == NULL )
	{
		...
		VideoPlaybackFlags_t nFlags = VideoPlaybackFlags::NO_AUDIO;	// FIXME: Allow?

		nFlags |= VideoPlaybackFlags::PRELOAD_VIDEO;
		if ( m_hScreenEntity->IsLooping() )
		{
			nFlags |= VideoPlaybackFlags::LOOP_VIDEO;
		}
		
		m_pVideoMaterial = g_pVideo->CreateVideoMaterial( szMaterialName, pFilename, "GAME", nFlags,
			VideoSystem::DETERMINE_FROM_FILE_EXTENSION, true );
		if ( m_pVideoMaterial == NULL )
		{
			// Portal 2 port: no video provider can open this movie (see
			// video/video_services); the screen keeps its background.
			static bool s_bWarned = false;
			if ( !s_bWarned )
			{
				s_bWarned = true;
				DevWarning( "Portal 2: vgui_movie_display could not open \"%s\"; no video provider supports it in this build\n", pFilename );
			}
			return false;
		}
	}
...
void CMovieDisplayScreen::Paint( void )
{
	...
	if ( m_bSlaved == false && m_pVideoMaterial == NULL )
	{
		BaseClass::Paint();
		return;
	}
