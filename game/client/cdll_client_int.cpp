...
// batches. Changes take effect at the next frame's render start.
static ConVar cl_render_start_graph( "cl_render_start_graph", "0", 0,
    "Client render-start simulation: 0 legacy order, 1 declared frame graph (serial), 2 declared "
    "frame graph (pooled)" );
...
void OnRenderStart()
{
	...
	// Main thread only; the sealed graph is reused while its shape is unchanged.
	static jobsystem::DeclaredFrameGraph *s_pRenderStartGraph = CreateDeclaredFrameGraph();
