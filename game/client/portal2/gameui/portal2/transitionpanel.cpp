...
void CBaseModTransitionPanel::ScanTilesForTransition()
{
	...
	if ( m_bTransitionActive )
	{
		if ( !bTransitionActive )
		{
			...
			// Portal 2 port: this vgui has no SetModalSubTreeShowMouse(); its surface
			// shows the mouse for a modal subtree whose popups accept mouse input.
...
bool CBaseModTransitionPanel::IsEffectEnabled()
{
	if ( !m_bAllowTransitions ||
		...
		// Portal 2 port: retail also disabled the effect in NVIDIA stereo mode;
		// this material system never renders stereo.
	{
...
void CBaseModTransitionPanel::StartPaint3D()
{
	...
	Portal2_SetStencilState( pRenderContext, state );

	pRenderContext->ClearBuffers( false, true, true );
	Portal2_OverrideDepthEnable( pRenderContext, true, true, true );
}
