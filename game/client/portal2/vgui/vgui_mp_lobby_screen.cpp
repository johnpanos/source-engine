...
// Portal 2 port: the 2010 build defined CVGUI_MP_ProgressSignScreen here. The
// retail client derives it from CVGUI_Base_ProgressSignScreen in
// vgui_mp_progress_screen.cpp, which now registers "mp_progress_sign".
...
void CVGUI_MP_LevelSelectScreen::UpdateLevelLabel( void )
{
	if ( g_pGameRules->IsMultiplayer() && PortalMPGameRules()->IsCoOp() && m_bInitializedLevelLabel )
	{
		...
		for ( int i = 0; i < nTotalLevels; i++ )
		{
			...
			// Reconstruction note: completion is read for player slot 0.
