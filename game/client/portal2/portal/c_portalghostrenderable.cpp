...
void GhostResetEverything( C_PortalGhostRenderable *pGhost )
{
	...
	if( pGhostedRenderable )
	{
		...
		Portal2_DisableCachedRenderBounds( pGhostedRenderable->RenderHandle(), true );
	...
	Portal2_DisableCachedRenderBounds( pGhost->RenderHandle(), true );
...
int C_PortalGhostRenderable::GetRenderFlags()
{
	...
	return Portal2_GetRenderFlags( m_hGhostedRenderable.Get() );
}
...
C_PortalGhostRenderable *C_PortalGhostRenderable::CreateGhostRenderable( C_BaseEntity *pEntity, C_Portal_Base2D *pPortal )
{
	...
	// Portal 2 port: CS:GO passed bRenderWithViewModels = false. This engine takes
	// the initial render group; the leaf system re-derives it from GetRenderGroup().
	...
	Portal2_DisableCachedRenderBounds( pNewGhost->RenderHandle(), true );
