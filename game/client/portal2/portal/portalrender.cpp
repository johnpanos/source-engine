...
//-----------------------------------------------------------------------------
// Portal 2 port: stands in for CS:GO's ICachedPerFrameMeshData. It records the
// portals and view inputs of the frame's portal quad mesh so recursive views
// rebuild exactly the same geometry (see RebuildCachedPortalQuadMesh()).
//-----------------------------------------------------------------------------
class CPortalQuadMeshReplayData : public ICachedPerFrameMeshData
{
public:
	CPortalQuadMeshReplayData( int nPortalCount, CPortalRenderable **ppPortals, const CPortalRenderable_FlatBasic::MeshViewInputs_t &inputs )
		: m_Inputs( inputs )
	{
		m_Portals.CopyArray( ppPortals, nPortalCount );
	}

	virtual void Free() { delete this; }

	bool References( CPortalRenderable *pPortal ) const
	{
		return m_Portals.Find( pPortal ) != m_Portals.InvalidIndex();
	}

	IMesh *Rebuild( IMatRenderContext *pRenderContext )
	{
		CUtlVector< ClampedPortalMeshRenderInfo_t > clampedInfos; // identical to the original build's
		return CPortalRenderable_FlatBasic::CreateMeshForPortals( pRenderContext, m_Portals.Count(), m_Portals.Base(), clampedInfos, m_Inputs );
	}

private:
	CUtlVector< CPortalRenderable * > m_Portals;
	CPortalRenderable_FlatBasic::MeshViewInputs_t m_Inputs;
};
...
CPortalRenderable::~CPortalRenderable( void )
{
	...
	s_PortalRender.InvalidateCachedPortalQuadMesh( this );
}
...
void CPortalRender::WaterRenderingHandler_PreReflection() const
{
	if( m_iViewRecursionLevel > 0 )
	{
		...
		Portal2_SetStencilState( pRenderContext, tempState );
	}
}
...
void CPortalRender::EnteredPortal( int nPlayerSlot, CPortalRenderable *pEnteredPortal )
{
	...
	else
	{
		...
		if ( pEnteredView )
		{
			...
			PixelVisibility_ShiftVisibilityViews( VIEW_MAIN, nExitPortalID );
			PixelVisibility_ShiftVisibilityViews( VIEW_3DSKY, nExitPortalID + 1 );

			// Shift visibility from the entered view to the main view
			PixelVisibility_ShiftVisibilityViews( pEnteredView->iPrimaryViewID, VIEW_MAIN );
			PixelVisibility_ShiftVisibilityViews( pEnteredView->iPrimaryViewID + 1, VIEW_3DSKY );
		}
	}
...
{	  
	...
	if ( m_iViewRecursionLevel == 0 )
	{
		...
		m_pCachedPortalQuadMeshData = NULL;
		CPortalRenderable_FlatBasic::MeshViewInputs_t meshInputs;
		CPortalRenderable_FlatBasic::CaptureMeshViewInputs( pRenderContext, meshInputs );
		pPortalQuadMesh = CPortalRenderable_FlatBasic::CreateMeshForPortals( pRenderContext, m_AllPortals.Count(), m_AllPortals.Base(), m_clampedPortalMeshRenderInfos, meshInputs );
		
		// Portal 2 port: record the inputs instead of CS:GO's cached mesh data.
		m_pCachedPortalQuadMeshData = new CPortalQuadMeshReplayData( m_AllPortals.Count(), m_AllPortals.Base(), meshInputs );
	...
	for ( int i = 0; i < portalRenderablesToDraw.Count(); i++ )
	{
		...
		if ( r_portalscissor.GetBool() )
		{
			...
			if ( assert_cast< CPortalRenderable_FlatBasic* >( pCurrentPortal )->ComputeClipSpacePortalCorners( clipSpacePortalCorners, matViewProj ) )
			{
				...
				Portal2_PushScissorRect( pRenderContext, int( flMinX ), int( flMinY ), int( flMaxX ), int( flMaxY ) );
				bPushedScissor = true;
			}
		}
		...
		//step 5, restore the stencil mask to the parent level
		{
			...
			int nStartIndex = portalsRenderablesToDrawVBIndex[i] * 6;
			int nIndexCount = 6;
			Portal2_OverrideDepthEnable( pRenderContext, true, true, false );	// Force depth writes on, but depth test off
			pPortalQuadMesh->Draw( nStartIndex, nIndexCount );
			if ( ( m_iViewRecursionLevel == 0 ) && ( m_clampedPortalMeshRenderInfos[ portalsRenderablesToDrawVBIndex[i] ].nStartIndex >= 0 ) )
			{
				// Draw near plane cap
		...
		if ( bPushedScissor )
		{
			Portal2_PopScissorRect( pRenderContext );
		}
	}
...
IMesh *CPortalRender::RebuildCachedPortalQuadMesh( IMatRenderContext *pRenderContext )
{
	if ( m_pCachedPortalQuadMeshData )
	{
		return static_cast< CPortalQuadMeshReplayData * >( m_pCachedPortalQuadMeshData )->Rebuild( pRenderContext );
	}

	// A portal in the recorded mesh was destroyed since it was built: build the
	// quads of the portals that remain, from the current view.
	CUtlVector< ClampedPortalMeshRenderInfo_t > clampedInfos;
	return CPortalRenderable_FlatBasic::CreateMeshForPortals( pRenderContext, m_AllPortals.Count(), m_AllPortals.Base(), clampedInfos );
}

void CPortalRender::InvalidateCachedPortalQuadMesh( CPortalRenderable *pPortal )
{
	if ( m_pCachedPortalQuadMeshData && static_cast< CPortalQuadMeshReplayData * >( m_pCachedPortalQuadMeshData )->References( pPortal ) )
	{
		m_pCachedPortalQuadMeshData->Free();
		m_pCachedPortalQuadMeshData = NULL;
	}
