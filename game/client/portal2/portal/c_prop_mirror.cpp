...
void C_Prop_Mirror::RenderPortalViewToBackBuffer( CViewRender *pViewRender, const CViewSetup &cameraView )
{
	...
	// Portal 2 port: CS:GO rendered through this reflecting view matrix. This
	// engine's views have only origin and angles, so the adapter reports the
	// reflection as unsupported and the mirror shows the unreflected view from
	// the mirrored camera (portalView.origin/angles above).
	...
	// Portal 2 port: CS:GO flipped the cull mode here because its view was
	// reflected. The view rendered here is not reflected, so the cull mode stays.
