...
BEGIN_PREDICTION_DATA( C_Portal_Base2D )
	...
	// Portal 2 port: this datamap has no FIELD_VECTOR4D; FIELD_QUATERNION also
	// copies the plane's first four floats (normal and dist).
...
{
	// Portal 2 port: CS:GO disabled its fast model-rendering path here so the
	// custom DrawModel() gets called. This engine has no fast path and always
	// calls DrawModel().
...
void C_Portal_Base2D::Simulate()
{
	...
	// Portal 2 port: CS:GO queued new ghost creation on the simulate call queue
	// (run after all entities simulate) so the new ghosts are not culled by the
	// fix-up below. This client has no simulate call queue, so the creations
	// are collected here and made once the fix-up is done.
	CUtlVector< C_BaseEntity * > ghostsToCreate;
	{
		...
		for( int i = m_hGhostingEntities.Count(); --i >= 0; )
		{
			...
			ghostsToCreate.AddToTail( pRenderable );
		}
	...
	for ( int i = 0; i != ghostsToCreate.Count(); ++i )
	{
		C_PortalGhostRenderable::CreateGhostRenderable( ghostsToCreate[i], this );
	}
