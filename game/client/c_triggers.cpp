...
public:
	...
	FORCEINLINE void Check( T *pEntity, bool bIgnoreTriggerSolidFlags )
	{
		...
		// Portal 2 port: this engine's ICollideable answers the trigger-flag
		// question directly (debris touches only debris triggers, triggers touch
		// no triggers) instead of CS:GO's GetRequiredTriggerFlags().
		if ( !bIgnoreTriggerSolidFlags && !m_pCollide->ShouldTouchTrigger( nTriggerSolidFlags ) )
			return;
...
void UntouchAllTriggerSoundOperator( C_BaseEntity *pEntity )
{
	for ( int j = 0; j < Q_ARRAYSIZE( g_szTriggerSoundOperatorVariables ); ++ j )
	{
		if ( g_bTriggerSoundOperatorVariablesSet[ j ] )
		{
			...
			Portal2Engine::SOSSetOpvarFloat( g_szTriggerSoundOperatorVariables[ j ], 0.0f );
		}
	}
}
...
void C_TriggerSoundOperator::UpdateSosVar( bool bTouchingNow )
{
	...
	Portal2Engine::SOSSetOpvarFloat( szVar, bTouchingNow ? 1.0f : 0.0f );
}
