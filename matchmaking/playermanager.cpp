...
#if !defined( NO_STEAM ) // Portal 2 port: friend requests are Steam-only (the CS:GO code left this unguarded)
...
#else
	// Portal 2 port: this engine has no LAN connectionless transport (the
	// Portal 2 title disables the player manager's searches in any case).
	PORTAL2_UNSUPPORTED( "LAN player discovery" );
#endif
}
...
void PlayerManager::OnEvent( KeyValues *pEvent )
{
	...
	else if ( !Q_stricmp( szName, "OnProfileLeaderboardData" ) )
	{
		...
		int nActiveSlot = MMPortal2::GetActiveSplitScreenPlayerSlot();
		MMPortal2::SetActiveSplitScreenPlayerSlot( iPlayerSlot );
