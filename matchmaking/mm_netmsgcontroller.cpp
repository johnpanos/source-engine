...
{
	// Query server info
	// Portal 2 port: the server game module's host adapter reports the local
	// server (CS:GO INetSupport::GetServerInfo); without one, no server runs here.
	...
	if ( si.m_bActive )
	{
		...
		// Portal 2 port: server network addresses and lobby/group exclusivity
		// come from INetSupport, which this engine lacks; the address keys
		// keep the template's empty values.
	...
	else if ( IVEngineClient *pIVEngineClient = g_pMatchExtensions->GetIVEngineClient() )
	{
		...
		char const *szLevelName = MMPortal2::GetLevelNameShort( pIVEngineClient );
...
KeyValues * CMatchNetworkMsgControllerBase::UnpackGameDetailsFromQOS( MM_GameDetails_QOS_t const *pvQosReply )
{
	...
	if ( iProtocol != MMPortal2::GetEngineBuildNumber() )
		return NULL;
	if ( 0 != iVersion )
		return NULL;
...
void CMatchNetworkMsgControllerBase::PackageGameDetailsForQOS( KeyValues *pSettings, CUtlBuffer &buf )
{
	...
	buf.PutInt( MMPortal2::GetEngineBuildNumber() );
	buf.PutInt( 0 );
