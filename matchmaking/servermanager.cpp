...
void CServerManager::OnEvent( KeyValues *pEvent )
{
	...
	// Portal 2 port: these events carry LAN connectionless packets, which
	// this engine does not deliver (see MM_PORTAL2_ONLINE_SESSIONS).
...
#else
	// Portal 2 port: this engine has no LAN connectionless transport for
	// server discovery; go straight to group search.
	PORTAL2_UNSUPPORTED( "LAN server discovery" );
	m_eState = STATE_GROUP_SEARCH;
#endif
}
...
#endif // Portal 2 port: never reached, the LAN search state is skipped
		}
	}
}
