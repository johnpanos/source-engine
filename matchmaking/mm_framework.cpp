...
{
	// Portal 2 port: the server game module's host adapter applies game
	// settings (CS:GO IServerGameDLL::ApplyGameSettings).
	IMatchServerHostPortal2 *pServerHost = g_pMatchExtensions->GetIMatchServerHost();
	if ( !pServerHost )
	{
		Warning( "CMatchFramework::ApplySettings: no server game module is registered.\n" );
		return;
	}
	pServerHost->ApplyGameSettings( keyValues );
}

// Portal 2 port: online sessions (Steam lobbies, LAN, reservations) are not
// built for this engine (see MM_PORTAL2_ONLINE_SESSIONS). A request for one
// is refused with the session error the UI reports as "not available".
static void MatchFramework_RefuseOnlineSession( char const *szRequest, KeyValues *pSettings )
{
	PORTAL2_UNSUPPORTED( "online matchmaking" );
	Warning( "%s: online session \"%s\" is not available in this build.\n", szRequest,
		pSettings->GetString( "system/network", "" ) );
	g_pMatchEventsSubscription->BroadcastEvent( new KeyValues( "OnMatchSessionUpdate", "state", "error", "error", "n/a" ) );
}
...
void CMatchFramework::CreateSession( KeyValues *pSettings )
{
	...
	else
	{
		...
		pMatchSessionNew = pSession;
#else
		MatchFramework_RefuseOnlineSession( "CreateSession", pSettings );
#endif
	}
...
void CMatchFramework::MatchSession( KeyValues *pSettings )
{
	...
	if ( !Q_stricmp( "joinsession", szAction ) )
	{
		...
		pMatchSessionNew = pSession;
#else
		MatchFramework_RefuseOnlineSession( "MatchSession", pSettings );
#endif
	}
