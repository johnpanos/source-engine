...
#include "fmtstr.h"
// Portal 2 port: SIGNONSTATE_* come from protocol.h (via mm_session.h) here.
...
void CMatchSessionOfflineCustom::Command( KeyValues *pCommand )
{
	...
	if ( !Q_stricmp( "QueueConnect", szCommand ) )
	{
		...
		if ( bAutoCloseSession )
		{
			...
			// Portal 2 port: this engine has no client reservations (CS:GO
			// INetSupport::UpdateClientReservation); the connect is unreserved.
			if ( uiReservationId )
				PORTAL2_UNSUPPORTED( "server reservation" );

			// Issue the connect command
			MMPortal2::StartLoadingScreenForCommand( g_pMatchExtensions->GetIVEngineClient(), CFmtStr( "connect %s", szConnectAddress ) );
