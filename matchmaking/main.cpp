...
// Portal 2 port: the framework is linked into the game module that owns it
// and handed out through MatchFramework_Portal2(), not exposed through the
// module's interface factory.
static CMatchFramework g_MatchFramework;
CMatchFramework *g_pMMF = &g_MatchFramework;

IMatchFramework *MatchFramework_Portal2()
{
...
{
	// Portal 2 port: this engine's localizer is the vgui one, and it has no
	// INetSupport (the server host adapter registers itself instead).
	{ VGUI_LOCALIZE_INTERFACE_VERSION,	(ExtFn_t) &CMatchExtensions::GetILocalize,			false },
...
bool CMatchFramework::Connect( CreateInterfaceFn factory )
{
	...
	// Portal 2 port: the owning game module has connected the tier libraries
	// and registers this library's ConVars with its own.
...
void CMatchFramework::Disconnect()
{
	...
	// Portal 2 port: the owning game module disconnects the tier libraries.
}
...
// Portal 2 port: COM_GetModDirectory() is the owning game module's
// (game/shared/achievementmgr.cpp), with the same -game/-defaultgamedir rule.
	...
	return
		( g_pMatchExtensions &&
		  ...
		  ( g_pMatchExtensions->GetIVEngineClient()->IsConnected() ||
			...
			MMPortal2::IsTransitioningToLoad() ) );
}
