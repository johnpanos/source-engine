...
void CDatacenter::RequestStart()
{
	...
	pJob->StartJob( NULL );
#else
	// Portal 2 port: without Steam or Xbox LIVE there is no datacenter
	// backend. Stay idle until the next regular update instead of retrying
	// (and failing) every frame.
	PORTAL2_UNSUPPORTED( "datacenter querying" );
