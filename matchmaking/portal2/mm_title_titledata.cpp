...
#define TD_ENTRY( szName, nTD, eDataType, numBytesOffset )                                         \
	{                                                                                              \
		TitleDataFieldsDescription_t aTDFD = { szName, TitleDataFieldsDescription_t::nTD,          \
		    TitleDataFieldsDescription_t::eDataType, int( numBytesOffset ) };                      \
...
#undef CFG
	...
	// Custom achievements data
	{
		...
		for ( int iComponent = 0; iComponent < TitleData2::kAchievement_SpreadTheLove_FriendsHuggedCount; ++ iComponent )
		{
			...
			TD_ENTRY( pszComponentName,	DB_TD2,	DT_uint64,	offsetof( TitleData2, ach_SpreadTheLove_FriendsHugged ) + iComponent * sizeof( uint64 ) /* Portal 2 port: offsetof needs a constant index in C++20 */ );
		}
	}
	{
		...
		for ( int iComponent = 0; iComponent < TitleData2::kAchievement_SpeedRunCoop_QualifiedRunsCount; ++ iComponent )
		{
			char *pszComponentName = new char[ numAchNameChars + 10 ];
			Q_snprintf( pszComponentName, numAchNameChars + 10, "%s[%d]", szAch, iComponent+1 );
			TD_ENTRY( pszComponentName,	DB_TD2,	DT_uint16,	offsetof( TitleData2, ach_SpeedRunCoop_MapsQualified ) + iComponent * sizeof( uint16 ) /* Portal 2 port: offsetof needs a constant index in C++20 */ );
		}
	}
...
TitleDlcDescription_t const * CMatchTitle::DescribeTitleDlcs()
{
	static TitleDlcDescription_t tdlcs[] = {
	    ...
	    { PORTAL2_DLCID_COOP_BOT_SKINS, PORTAL2_DLC_APPID_COOP_BOT_SKINS,
	        PORTAL2_DLC_PKGID_COOP_BOT_SKINS, "DLC.0x12" },
	    { PORTAL2_DLCID_COOP_BOT_HELMETS, PORTAL2_DLC_APPID_COOP_BOT_HELMETS,
	        PORTAL2_DLC_PKGID_COOP_BOT_HELMETS, "DLC.0x13" },
	    { PORTAL2_DLCID_COOP_BOT_ANTENNA, PORTAL2_DLC_APPID_COOP_BOT_ANTENNA,
	        PORTAL2_DLC_PKGID_COOP_BOT_ANTENNA, "DLC.0x14" },
	    // END MARKER
	    { 0, 0, 0 } };
