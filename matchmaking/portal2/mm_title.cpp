...
{
	if ( vgui::ILocalize *pLocalize = g_pMatchExtensions->GetILocalize() ) // Portal 2 port: this engine's localizer type
	{
...
void CMatchTitle::PrepareClientForConnect( KeyValues *pSettings )
{
	...
	for ( int k = 0; k < numPlayers; ++ k )
	{
		...
		// Set "name"
		// Portal 2 port: this engine has one local player and no split-screen
		// convar slots; the first player's name is the "name" convar.
		static ConVarRef s_cl_name( "name" );
		char const *szName = pPlayerLocal->GetName();
		if ( k == 0 )
			s_cl_name.SetValue( szName );
		else
			PORTAL2_UNSUPPORTED( "split-screen player naming" );

		// Set "networkid_force"
#ifdef _X360 // Portal 2 port: console-only network id
		if ( IsX360() )
		{
...
bool CMatchTitle::StartServerMap( KeyValues *pSettings )
{
	...
	// Portal 2 port: through the server game module's host adapter
	// (CS:GO IServerGameDLL::ApplyGameSettings).
	IMatchServerHostPortal2 *pServerHost = g_pMatchExtensions->GetIMatchServerHost();
	if ( !pServerHost )
		return false;
	pServerHost->ApplyGameSettings( pGameDllReserve );
...
static void SendPreConnectClientDataToServer( int nSlot )
{
	...
	int nRestoreSlot = MMPortal2::GetActiveSplitScreenPlayerSlot();
	MMPortal2::SetActiveSplitScreenPlayerSlot( nSlot );
	g_pMatchExtensions->GetIVEngineClient()->ServerCmdKeyValues( pPreConnectData );
	MMPortal2::SetActiveSplitScreenPlayerSlot( nRestoreSlot );
}
...
void CMatchTitle::OnEvent( KeyValues *pEvent )
{
	...
	else if ( !Q_stricmp( szEvent, "OnPlayerAward" ) )
	{
		...
		if ( !Q_stricmp( szAward, "ACH.HI_FIVE_YOUR_PARTNER" ) )
		{
			...
			int nActiveSlot = MMPortal2::GetActiveSplitScreenPlayerSlot();
			MMPortal2::SetActiveSplitScreenPlayerSlot( XBX_GetSlotByUserId( iCtrlr ) );
			...
			MMPortal2::SetActiveSplitScreenPlayerSlot( nActiveSlot );
		}
