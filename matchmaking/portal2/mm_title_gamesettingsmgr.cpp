...
#ifdef _X360 // Portal 2 port: the Xbox installer is used only by console builds
...
public:
	...
	// Portal 2 port: the CS:GO interface added the official-server and pass arguments.
	...
	// Portal 2 port: IMatchTitleGameSettingsMgr grew these for the CS:GO title
	// (map groups, team lobbies, X360 matchmaking formulas, team properties).
	// Portal 2 has none of those concepts; the answers below are the title's.
	virtual void SetBspnameFromMapgroup( KeyValues *pSettings ) {} // "game/map" is always the bsp name
	virtual KeyValues * ExtendTeamLobbyToGame( KeyValues *pSettings ) { return NULL; } // no team lobbies
	virtual char const * GetFormulaAverage( int index ) { return NULL; } // no MatchSystem.360.res formulas
	virtual void UpdateTeamProperties( KeyValues *pCurrentSettings, KeyValues *pTeamProperties ) {} // no team properties
};
...
{
	// Query server info
	// Portal 2 port: from the server game module's host adapter (CS:GO INetSupport).
	MatchServerInfoPortal2_t si = {};
	if ( IMatchServerHostPortal2 *pServerHost = g_pMatchExtensions->GetIMatchServerHost() )
		pServerHost->GetServerInfo( &si );
	...
	if ( !si.m_bActive )
	{
		...
		{
			szMapName = MMPortal2::GetLevelNameShort( pIVEngineClient );
		}
	}
	if ( !szMapName )
		szMapName = "";
