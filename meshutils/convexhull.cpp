...
// Portal 2 port: bi32x4, CrossProductSIMD, Normalized3SIMD and g_SIMD_Identity.
#include "portal2_ssemath_compat.h"

// Portal 2 port: CS:GO mathlib_base.cpp SIMD polygon helpers, kept local to
// this unit because only the convex hull builder uses them.
namespace
{
	enum
	{
	...
	FORCEINLINE fltx4 AbsSIMD( const fltx4 &x )
	{
		return AndSIMD( x, LoadAlignedSIMD( g_SIMD_clear_signmask ) );
	}

	FORCEINLINE fltx4 NegateSIMD( const fltx4 &x )
	{
		return SubSIMD( Four_Zeros, x );
	}

	// A large square polygon on the plane (xyz normal, w distance).
	void PolyFromPlane_SIMD( fltx4 *pOutVerts, const fltx4 &plane, float fHalfScale = 9000.0f )
	{
		// Build a unit vector along an axis that is not the plane normal's major axis.
		fltx4 f4Abs = AbsSIMD( plane );
		fltx4 x = SplatXSIMD( f4Abs );
		fltx4 y = SplatYSIMD( f4Abs );
		fltx4 z = SplatZSIMD( f4Abs );
		fltx4 max = MaxSIMD( MaxSIMD( x, y ), z );

		// If Z is the biggest component use 1 0 0, otherwise 0 0 1.
		bi32x4 fIsMax = CmpEqSIMD( max, f4Abs );
		fltx4 fIsZMax = SplatZSIMD( (fltx4)fIsMax );
		...
		// Remove the component of vup along the normal and make it a unit vector.
		fltx4 v = Dot3SIMD( vup, normal );
		vup = MaddSIMD( NegateSIMD( v ), normal, vup );
		vup = Normalized3SIMD( vup );

		// Center of the poly is at normal * dist; the third basis vector is in the plane too.
	...
	// Clips a polygon to the front side of the plane; returns the output vertex count.
	int ClipPolyToPlane_SIMD( fltx4 *pInVerts, int nVertCount, fltx4 *pOutVerts, const fltx4 &plane, float fOnPlaneEpsilon = 0.1f )
	{
		vec_t *dists = (vec_t *)stackalloc( sizeof( vec_t ) * nVertCount * 4 );
		...
		{
			fltx4 dot = SubSIMD( Dot3SIMD( pInVerts[i], plane ), f4Dist );
		...
		// Where a normal component is exactly 1 or -1, the split point uses dist or -dist.
		fltx4 f4OneMask = (fltx4)CmpEqSIMD( plane, Four_Ones );
		fltx4 f4mOneMask = (fltx4)CmpEqSIMD( plane, NegateSIMD( Four_Ones ) );
		fltx4 f4AllMask = OrSIMD( f4OneMask, f4mOneMask );
		f4OneMask = AndSIMD( f4OneMask, f4Dist );
		f4mOneMask = AndSIMD( f4mOneMask, NegateSIMD( f4Dist ) );
		fltx4 f4AllValue = OrSIMD( f4OneMask, f4mOneMask );
		...
		for ( i = 0; i < nVertCount; i++ )
		{
			...
			const fltx4 &p2 = pInVerts[( i + 1 ) % nVertCount];
			fltx4 f4Dot = ReplicateX4( dists[i] / ( dists[i] - dists[i + 1] ) );
			fltx4 f4Result = MaddSIMD( f4Dot, SubSIMD( p2, p1 ), p1 );
			pOutVerts[nOutCount++] = MaskedAssign( (bi32x4)f4AllMask, f4AllValue, f4Result );
		}
