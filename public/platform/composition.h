...
{
public:
	template <CapabilityContract Contract> static CapabilityKey For() noexcept
	{
...
private:
	template <typename Contract> inline static const char CapabilityToken = 0;

	CapabilityKey( const void *token, std::string_view name ) noexcept
...
{
	template <CapabilityContract Contract> static DependencyDescriptor Required() noexcept
	{
		return { CapabilityKey::For<Contract>(), DependencyRequirement::Required };
	}

	template <CapabilityContract Contract> static DependencyDescriptor Optional() noexcept
	{
...
private:
	ProviderInstance(
	    std::unique_ptr<IProviderLifecycle> owner, std::vector<CapabilityExport> exports )
...
{
public:
	template <CapabilityContract Contract> Contract *Get() const noexcept
	{
...
public:
	...
	template <typename Provider, CapabilityContract... Contracts, typename Factory>
	    requires std::derived_from<Provider, IProviderLifecycle> &&
	             ( std::derived_from<Provider, Contracts> && ... )
	static ProviderDescriptor Define(
	    std::string name, std::vector<DependencyDescriptor> dependencies, Factory factory )
	{
		std::vector<CapabilityKey> capabilities{ CapabilityKey::For<Contracts>()... };
		FactoryFunction erased = [factory = std::move( factory )](
		                             const DependencyView &view ) mutable
...
private:
	using FactoryFunction =
	    std::function<foundation::Expected<detail::ProviderInstance, ProviderError>(
...
public:
	...
	[[nodiscard]] foundation::Expected<void, CompositionError> AddProvider(
