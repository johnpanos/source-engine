...
struct TitleAvatarAwardsDescription_t
{
	...
	char const *
	    m_szTitleDataBitfieldStatName; // Name of a bitfield in title data storage representing whether avatar award has been earned
};
...
struct TitleDlcDescription_t
{
	...
	char const *
	    m_szTitleDataBitfieldStatName; // Name of a bitfield in title data storage representing whether dlc has been discovered installed
};
...
enum TitleSettingsFlags_t
{
	...
	MATCHTITLE_INVITE_ONLY_SINGLE_USER =
	    ( 1 << 4 ), // Accepted game invite forcefully disables splitscreen (e.g.: 1-on-1 games)
	MATCHTITLE_VOICE_INGAME =
	    ( 1 << 5 ), // When in active gameplay lobby system doesn't transmit voice
	MATCHTITLE_XLSPPATCH_SUPPORTED = ( 1 << 6 ), // Title supports xlsp patch containers
	MATCHTITLE_PLAYERMGR_ALLFRIENDS =
	    ( 1 << 7 ), // Player manager by default fetches only friends for same game, this will force all friends to be fetched
	MATCHTITLE_PLAYERMGR_FRIENDREQS =
	    ( 1 << 8 ), // Player manager by default fetches only real friends, this will force friend requests to also be fetched
};
...
public:
	...
	virtual void ExtendLobbyDetailsTemplate(
	    KeyValues * pDetails, char const *szReason, KeyValues *pFullSettings ) = 0;

	// Extends game settings update packet for lobby transition,
	// either due to a migration or due to an endgame condition
	virtual void ExtendGameSettingsForLobbyTransition(
	    KeyValues * pSettings, KeyValues * pSettingsUpdate, bool bEndGame ) = 0;
	...
	virtual KeyValues *RollupGameDetails(
	    KeyValues * pDetails, KeyValues * pRollup, KeyValues * pQuery ) = 0;
	...
	virtual KeyValues *DefineDedicatedSearchKeys(
	    KeyValues * pSettings, bool bNeedOfficialServer, int nSearchPass ) = 0;
	...
	virtual void ExtendGameSettingsUpdateKeys(
	    KeyValues * pSettings, KeyValues * pUpdateDeleteKeys ) = 0;
	...
	// be modified
	virtual void ExecuteCommand( KeyValues * pCommand, KeyValues * pSessionSystemData,
	    KeyValues * pSettings, KeyValues * *ppPlayersUpdated ) = 0;
	...
	virtual KeyValues *PrepareTeamLinkForGame(
	    KeyValues * pSettingsLocal, KeyValues * pSettingsRemote ) = 0;
	...
	virtual KeyValues *PrepareClientLobbyForMigration(
	    KeyValues * pSettingsLocal, KeyValues * pMigrationInfo ) = 0;
	...
	virtual KeyValues *PrepareClientLobbyForGameDisconnect(
	    KeyValues * pSettingsLocal, KeyValues * pDisconnectInfo ) = 0;
	...
	virtual void UpdateTeamProperties(
	    KeyValues * pCurrentSettings, KeyValues * pTeamProperties ) = 0;
};
