...
public:
	...
	{
		if ( m_Suites.empty() ) return Invalid( "no required suites selected" );
		for ( std::size_t i = 0; i < m_Suites.size(); ++i )
		{
			if ( m_Suites[i].name.empty() || !m_Suites[i].run ) return Invalid( "missing suite name or callback" );
			for ( std::size_t j = 0; j < i; ++j )
				if ( m_Suites[i].name == m_Suites[j].name ) return Invalid( "duplicate required suite" );
		}
		return {};
	}
	void Shutdown() noexcept override {}
...
private:
	...
	{
		return foundation::MakeUnexpected( ProviderError{ platform::ProviderErrorCode::InitializationFailed, detail } );
	}
...
int RunLinkedSuites( std::span< const Suite > suites, ITestReport &report )
{
	...
	auto execution = composition.AddProvider(
	    platform::ProviderDescriptor::Define< ExecutionProvider, ITestExecution >(
	        ...
	        [suites]( const platform::DependencyView &deps ) -> Expected< std::unique_ptr< ExecutionProvider >, ProviderError > {
		        return std::make_unique< ExecutionProvider >( suites, *deps.Get< IReporting >() );
	        } ) );
	auto reporting = composition.AddProvider(
	    platform::ProviderDescriptor::Define< ReportProvider, IReporting >(
	        "test-reporter", {},
	        [&report]( const platform::DependencyView & ) -> Expected< std::unique_ptr< ReportProvider >, ProviderError > {
		        return std::make_unique< ReportProvider >( report );
	        } ) );
	if ( !execution || !reporting ) { report.Error( "provider registration failed" ); return 1; }
	auto started = composition.Start();
	if ( !started ) { report.Error( started.Error().detail ); return 1; }
