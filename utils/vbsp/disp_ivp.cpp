...
MaterialSystemMaterial_t GetMatIDFromDisp( mapdispinfo_t *pMapDisp )
{
	...
	MaterialSystemMaterial_t matID = FindOriginalMaterial(
	    TexDataStringTable_GetString( pTexData->nameStringTableID ), NULL, true );
	return matID;
}
...
void Disp_AddCollisionModels(
    CUtlVector<CPhysCollisionEntry *> &collisionList, dmodel_t *pModel, int contentsMask )
{
	int dispIndex;
		...
		for ( int listIndex = 0; listIndex < gDispGridList[grid].dispList.Count(); listIndex++ )
		{
			...
			if ( triCount >= 65536 )
			{
				...
				CPhysCollide *pCollide =
				    physcollision->ConvertPolysoupToCollide( pTerrainPhysics, false );
				if ( pCollide )
				{
			...
			for ( int iTri = 0; iTri < nTriCount; ++iTri )
			{
				...
				physcollision->PolysoupAddTriangle(
				    pTerrainPhysics, tmpVerts[0], tmpVerts[1], tmpVerts[2], nMaterialIndex );
			}
		}
...
public:
	...
	virtual void GetTrianglesInSphere(
	    void *userData, const Vector &center, float radius, virtualmeshtrianglelist_t *pList );
...
void CDispMeshEvent::GetTrianglesInSphere(
    void *userData, const Vector &center, float radius, virtualmeshtrianglelist_t *pList )
{
	Assert( userData == ( (void *)this ) );
	pList->triangleCount = m_indexCount / 3;
		...
		if ( 1 )
		{
			...
			for ( int j = 0; j < triCount; j++ )
			{
				...
				if ( v0 == v1 || v1 == v2 || v2 == v0 )
				{
					...
					Error(
					    "Can't compile displacement physics, exiting.  Texture is %s\n", pMatName );
				}
			}
		}
	...
	for ( int i = 0; i < header.numDisplacements; i++ )
	{
		if ( virtualMeshes[i] )
		{
			...
			unsigned int outSize =
			    physcollision->CollideWrite( dispBuf.Base(), virtualMeshes[i], false );
			Assert( outSize == testSize );
			buf.Put( dispBuf.Base(), outSize );
		}
	}
	g_PhysDispSize =
	    totalSize + sizeof( dphysdisp_t ) + ( sizeof( unsigned short ) * header.numDisplacements );
