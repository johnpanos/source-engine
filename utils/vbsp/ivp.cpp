...
void DumpCollideToGlView( CPhysCollide *pCollide, const char *pFilename )
{
	...
	for ( int i = 0; i < triCount; i++ )
	{
		fprintf( fp, "3\n" );
		fprintf(
		    fp, "%6.3f %6.3f %6.3f 1 0 0\n", outVerts[vert].x, outVerts[vert].y, outVerts[vert].z );
		vert++;
		fprintf(
		    fp, "%6.3f %6.3f %6.3f 0 1 0\n", outVerts[vert].x, outVerts[vert].y, outVerts[vert].z );
		vert++;
		fprintf(
		    fp, "%6.3f %6.3f %6.3f 0 0 1\n", outVerts[vert].x, outVerts[vert].y, outVerts[vert].z );
		vert++;
	}
	fclose( fp );
...
void CPhysCollisionEntry::DumpCollideFileName(
    const char *pName, int modelIndex, CTextBuffer *pTextBuffer )
{
	char tmp[128];
...
CPhysCollisionEntrySolid::CPhysCollisionEntrySolid(
    CPhysCollide *pCollide, const char *pMaterialName, float mass )
...
void CPhysCollisionEntrySolid::DumpCollide(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	DumpCollideFileName( "collide", modelIndex, pTextBuffer );
}

void CPhysCollisionEntrySolid::WriteToTextBuffer(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
...
CPhysCollisionEntryStaticSolid ::CPhysCollisionEntryStaticSolid(
    CPhysCollide *pCollide, int contentsMask )
    : CPhysCollisionEntry( pCollide ), m_contentsMask( contentsMask )
{
}

void CPhysCollisionEntryStaticSolid::DumpCollide(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	char tmp[128];
...
void CPhysCollisionEntryStaticSolid::WriteToTextBuffer(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
...
CPhysCollisionEntryStaticMesh::CPhysCollisionEntryStaticMesh(
    CPhysCollide *pCollide, const char *pMaterialName )
    : CPhysCollisionEntry( pCollide )
{
	m_pMaterial = pMaterialName;
}

void CPhysCollisionEntryStaticMesh::DumpCollide(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	char tmp[128];
...
void CPhysCollisionEntryStaticMesh::WriteToTextBuffer(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	pTextBuffer->WriteText( "staticsolid {\n" );
	pTextBuffer->WriteIntKey( "index", collideIndex );
...
public:
	...
	CPhysCollisionEntryFluid( CPhysCollide *pCollide, const char *pSurfaceProp, float damping,
	    const Vector &normal, float dist, int nContents );
...
CPhysCollisionEntryFluid::CPhysCollisionEntryFluid( CPhysCollide *pCollide,
    const char *pSurfaceProp, float damping, const Vector &normal, float dist, int nContents )
    : CPhysCollisionEntry( pCollide )
{
...
void CPhysCollisionEntryFluid::DumpCollide(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	char tmp[128];
...
void CPhysCollisionEntryFluid::WriteToTextBuffer(
    CTextBuffer *pTextBuffer, int modelIndex, int collideIndex )
{
	...
	pTextBuffer->WriteFloatArrayKey(
	    "currentvelocity", vec3_origin.Base(), 3 ); // write out water velocity
	pTextBuffer->WriteText( "}\n" );
}
    ...
    int brushnumber, float shrink, CPhysCollide *pCollideTest, float shrinkMinimum )
{
	...
	for ( int i = 0; i < dbrushes[brushnumber].numsides; i++ )
	{
		...
		{
			Vector start = physcollision->CollideGetExtent(
			    pCollideTest, vec3_origin, vec3_angle, pplane->normal );
			Vector end = physcollision->CollideGetExtent(
			    pCollideTest, vec3_origin, vec3_angle, -pplane->normal );
			...
			if ( fabs( thick ) < shrinkMinimum )
			{
				...
				Warning( "Can't shrink brush %d, plane %d (%.2f, %.2f, %.2f)\n", brushnumber,
				    pside->planenum, pplane->normal[0], pplane->normal[1], pplane->normal[2] );
#endif
				shrinkThisPlane = 0;
			}
		}
		AddListPlane( &temp, pplane->normal[0], pplane->normal[1], pplane->normal[2],
		    pplane->dist - shrinkThisPlane );
	}
	...
	for ( int brushnumber = 0; brushnumber < numbrushes; brushnumber++ )
	{
		if ( IsBrushReferenced( brushnumber ) )
		{
			...
			if ( m_shrink != 0 )
			{
				...
				CPhysCollide *pUnshrunkCollide =
				    physcollision->ConvertConvexToCollide( &pConvex, 1 );
				pBrushConvex =
				    BuildConvexForBrush( brushnumber, m_shrink, pUnshrunkCollide, m_shrink * 3 );
...
//			*fullname -
//-----------------------------------------------------------------------------
void GetWaterTextureName(
    char const *mapname, char const *materialname, int waterdepth, char *fullname )
{
	char temp[512];
...
void EmitWaterMaterialFile( WaterTexInfo *wti )
{
	...
	GetWaterTextureName(
	    mapbase, wti->m_MaterialName.String(), (int)wti->m_nWaterDepth, waterTextureName );

	// Convert to string
	char szDepth[32];
	sprintf( szDepth, "%i", wti->m_nWaterDepth );
	CreateMaterialPatch(
	    wti->m_MaterialName.String(), waterTextureName, "$waterdepth", szDepth, PATCH_INSERT );
}
...
int FindOrCreateWaterTexInfo( texinfo_t *pBaseInfo, float depth )
{
	...
	char const *name =
	    TexDataStringTable_GetString( GetTexData( pBaseInfo->texdata )->nameStringTableID );
...
static void WriteFogVolumeIDs( dmodel_t *pModel )
{
	...
	for ( i = pModel->firstface; i < pModel->firstface + pModel->numfaces; i++ )
	{
		...
		if ( pFaceNode )
		{
			if ( ( pTexInfo->flags & SURF_WARP ) && pFaceNode->planenum == PLANENUM_LEAF &&
			     pFaceNode->diskId >= 0 )
			{
...
static bool PortalCrossesWater( waterleaf_t &baseleaf, portal_t *portal )
{
	...
	{
		int side =
		    WindingOnPlaneSide( portal->winding, baseleaf.surfaceNormal, baseleaf.surfaceDist );
...
static int FindOrCreateLeafWaterData( float surfaceZ, float minZ, int surfaceTexInfoID )
{
	...
	for ( i = 0; i < numleafwaterdata; i++ )
	{
		...
		if ( pLeafWaterData->surfaceZ == surfaceZ && pLeafWaterData->minZ == minZ &&
...
static void Flood_FindConnectedWaterVolumes_r(
    CUtlVector<node_t *> &list, node_t *pLeaf, waterleaf_t &baseleaf, leafbitarray_t &visited )
{
	// already visited, or not the same water contents
	if ( pLeaf->diskId < 0 || visited.Get( pLeaf->diskId ) ||
	     !( pLeaf->contents & ( baseleaf.pNode->contents & MASK_WATER ) ) )
		return;
	...
	for ( i = 0; i < list.Count(); i++ )
	{
		...
		if ( waterAreaList.Count() )
		{
			...
			if ( tmp.waterLeafData.surfaceTexInfo < 0 )
			{
				...
				tmp.waterLeafData.surfaceTexInfo =
				    FirstWaterTexinfo( list[i].pNode->brushlist, tmp.contents );
			}
			tmp.depthTexinfo =
			    FindOrCreateWaterTexInfo( &texinfo[tmp.waterLeafData.surfaceTexInfo], waterDepth );
			tmp.fogVolumeIndex = FindOrCreateLeafWaterData( tmp.waterLeafData.surfaceDist,
			    tmp.waterLeafData.minZ, tmp.waterLeafData.surfaceTexInfo );
...
static void ConvertWaterModelToPhysCollide( CUtlVector<CPhysCollisionEntry *> &collisionList,
    int modelIndex, float shrinkSize, float mergeTolerance )
{
	...
	for ( int i = 0; i < g_WaterModels.Count(); i++ )
	{
		...
		if ( pCollide )
		{
			...
			if ( !waterModel.waterLeafData.hasSurface )
			{
				...
				Vector top = physcollision->CollideGetExtent(
				    pCollide, vec3_origin, vec3_angle, waterModel.waterLeafData.surfaceNormal );
			...
			CPhysCollisionEntryFluid *pCollisionEntryFuild = new CPhysCollisionEntryFluid( pCollide,
			    pSurfaceProp, damping, waterModel.waterLeafData.surfaceNormal,
			    waterModel.waterLeafData.surfaceDist, waterModel.contents );
...
static void ConvertWorldBrushesToPhysCollide( CUtlVector<CPhysCollisionEntry *> &collisionList,
    float shrinkSize, float mergeTolerance, int contentsMask )
{
	CPlaneList planes( shrinkSize, mergeTolerance );
...
static void BuildWorldPhysModel(
    CUtlVector<CPhysCollisionEntry *> &collisionList, float shrinkSize, float mergeTolerance )
{
	ConvertWorldBrushesToPhysCollide( collisionList, shrinkSize, mergeTolerance, MASK_SOLID );
	ConvertWorldBrushesToPhysCollide(
	    collisionList, shrinkSize, mergeTolerance, CONTENTS_PLAYERCLIP );
	ConvertWorldBrushesToPhysCollide(
	    collisionList, shrinkSize, mergeTolerance, CONTENTS_MONSTERCLIP );

	if ( !g_bNoVirtualMesh && Disp_HasPower4Displacements() )
	{
		Warning( "WARNING: Map using power 4 displacements, terrain physics cannot be compressed, "
		         "map will need additional memory and CPU.\n" );
...
static void ConvertModelToPhysCollide( CUtlVector<CPhysCollisionEntry *> &collisionList,
    int modelIndex, int contents, float shrinkSize, float mergeTolerance )
{
	int i;
	CPlaneList planes( shrinkSize, mergeTolerance );
	...
	CPhysCollide *pCollide =
	    physcollision->ConvertConvexToCollideParams( planes.m_convex.Base(), count, params );
...
void EmitPhysCollision()
{
	...
	{
		physcollision =
		    (IPhysicsCollision *)physicsFactory( VPHYSICS_COLLISION_INTERFACE_VERSION, NULL );
	}
